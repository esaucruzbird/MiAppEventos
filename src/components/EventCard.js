...
export default function EventCard({ event, onPress, onEdit, onDelete, onToggleRSVP, userIsGoing, isAdmin }) {
  ...
  return (
    <TouchableOpacity onPress={() => onPress(event)} style={[styles.card, past && styles.pastCard]}>
      ...
      <Text numberOfLines={2} style={[styles.desc, past && styles.pastText]}>{event.description}</Text>
      ...
      <View style={styles.actions}>
        ...
        {isAdmin && (
          <View style={styles.adminBtns}>
            <TouchableOpacity onPress={() => onEdit(event)} style={styles.btnSecondary}><Text>Editar</Text></TouchableOpacity>
            <TouchableOpacity onPress={() => onDelete(event)} style={styles.btnDanger}><Text>Eliminar</Text></TouchableOpacity>
          </View>
        )}
      </View>
    </TouchableOpacity>
